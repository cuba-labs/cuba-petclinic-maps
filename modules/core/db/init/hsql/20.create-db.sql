-- begin PETCLINIC_VISIT
alter table PETCLINIC_VISIT add constraint FK_PETCLINIC_VISIT_PET foreign key (PET_ID) references PETCLINIC_PET(ID)^
alter table PETCLINIC_VISIT add constraint FK_PETCLINIC_VISIT_CLINIC foreign key (CLINIC_ID) references PETCLINIC_CLINIC(ID)^
create index IDX_PETCLINIC_VISIT_PET on PETCLINIC_VISIT (PET_ID)^
create index IDX_PETCLINIC_VISIT_CLINIC on PETCLINIC_VISIT (CLINIC_ID)^
-- end PETCLINIC_VISIT
-- begin PETCLINIC_OWNER
alter table PETCLINIC_OWNER add constraint FK_PETCLINIC_OWNER_DISTRICT foreign key (DISTRICT_ID) references PETCLINIC_DISTRICT(ID)^
create index IDX_PETCLINIC_OWNER_DISTRICT on PETCLINIC_OWNER (DISTRICT_ID)^
-- end PETCLINIC_OWNER
-- begin PETCLINIC_PET
alter table PETCLINIC_PET add constraint FK_PETCLINIC_PET_TYPE foreign key (TYPE_ID) references PETCLINIC_PET_TYPE(ID)^
alter table PETCLINIC_PET add constraint FK_PETCLINIC_PET_OWNER foreign key (OWNER_ID) references PETCLINIC_OWNER(ID)^
create unique index IDX_PETCLINIC_PET_ID_UNQ on PETCLINIC_PET (IDENTIFICATION_NUMBER, DELETE_TS) ^
create index IDX_PETCLINIC_PET_TYPE on PETCLINIC_PET (TYPE_ID)^
create index IDX_PETCLINIC_PET_OWNER on PETCLINIC_PET (OWNER_ID)^
-- end PETCLINIC_PET
-- begin PETCLINIC_VET_SPECIALTY_LINK
alter table PETCLINIC_VET_SPECIALTY_LINK add constraint FK_VETSPE_VET foreign key (VET_ID) references PETCLINIC_VET(ID)^
alter table PETCLINIC_VET_SPECIALTY_LINK add constraint FK_VETSPE_SPECIALTY foreign key (SPECIALTY_ID) references PETCLINIC_SPECIALTY(ID)^
-- end PETCLINIC_VET_SPECIALTY_LINK
